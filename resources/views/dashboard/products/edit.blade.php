...
@section('content')
    ...
    <form action="{{ route("products.update", $product) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <select id="categorie_id" class="form-control" name="categorie_id">
                ...
                @foreach ($categories as $item)
                    <option value="{{ $item->id }}" {{ ($product->categorie_id === $item->id) ? "selected" : "" }}>{{ $item->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input id="stock" class="form-control" type="number" min="0" name="stock" value="{{ $product->stock }}">
        </div>
        <div class="form-group">
